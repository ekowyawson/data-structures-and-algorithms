...
const removeThree = (idx, arr) => {
  if (idx >= 0 && idx + 2 < arr.length) {
    ...
    return('Index is out of bounds or does not have enough elements to remove.');
  }
...
const howMuchPencil = (str) => {
  let result = [];
  // Use a loop to iterate through the string and create successive substrings
...
const listFoods = (recipe) => {
  let result = [];
  const units = ['medium-sized', 'pounds', 'gallons', 'cups', 'pound'];
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    ...
    for (const unit of units) {
      ...
      foodItem = unitIndex !== -1
        ? ingredient.slice(unitIndex + unit.length).trim()
        : ingredient;
...
const splitFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    const words = recipe.ingredients[i].split(' ');
    const unitIndex = words.findIndex(
      word => units.includes(word.toLowerCase())
    );
    const foodItem = unitIndex !== -1
      ? words.slice(unitIndex + 1).join(' ')
...
const stepActions = (recipe) => {
  ...
  for (let i = 0; i < recipe.steps.length; i++) {
    const words = recipe.steps[i].split(' ');
    if (words.length > 0) {
      result.push(words[0]);
    }
  }
