...
const addPurchases = (arr) => {
  const total = arr.reduce(
    (accumulator, purchase) => accumulator + purchase.purchasePrice, 0);
...
const returnNames = (arr) => {
  const namesArray = arr.reduce((acc, char) => {
    acc.push(char.name);
    return acc;
  }, []);
...
const reversedString = (str) => {
  // Use reduce to reverse the order of characters in the string
  const reversed = str.split('').reduce(
    (acc, char) => char + acc, ''
  );
...
const countNumberOfChildren = (arr) => {
  const totalChildren = arr.reduce((acc, char) => {
    return acc += char.children ? char.children.length : 0;
  }, 0);
...
const calculateAverage = (arr) => {
  ...
  const { count, sum } = arr.reduce((acc, curNum) => {
    return {
      count: acc.count + 1,
      sum: acc.sum + curNum,
    };
...
const countPrimeNumbers = (arr) => {
  return arr.reduce((count, number) => {
    return isPrime(number) ? count + 1 : count;
  }, 0);
};
...
const extractStats = (snorlaxData) => {
  const stats = snorlaxData.stats.reduce((acc, stat) => {
...
const extractChildren = (arr) => {
  const filterChars = arr.filter((char) => {
    return char ? char.name.toLowerCase().includes('a') : null;
  });

  const childNames = filterChars.reduce((acc, char) => {
    if (char.children) {
