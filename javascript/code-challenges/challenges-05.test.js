...
const hasChildrenValues = (arr, character) => {
  const char = arr.find(char => char.name === character);
  const charVal = Object.values(char || {});
  return charVal.includes(character) && Array.isArray(char.children) && char.children.length > 0;
};
...
const hasChildrenEntries = (arr, character) => {
  const getCharacter = arr.find(char => char.name === character);
  const characterEntries = Object.entries(getCharacter || {});
  return characterEntries.some(([key, value]) => key === 'children' && Array.isArray(value) && value.length > 0);
};
