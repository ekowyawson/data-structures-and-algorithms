...
const count = (target, input) => {
  const totalCount = input.reduce((acc, currentArray) => {
    const occurrencesInArray = currentArray.filter(num => num === target).length;
    return acc + occurrencesInArray;
  }, 0);
...
const totalSum = (input) => {
  const totalSum = input.reduce((acc, currentArray) => {
    const sumOfArray = currentArray.reduce((arraySum, num) => arraySum + num, 0);
    return acc + sumOfArray;
  }, 0);
  return totalSum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  const result = input.map(subArray => {
    const filteredArray = subArray.filter(num => typeof num === 'number' && num % 5 === 0);
...
let findMaleAndFemale = (data) => {
  ...
  const resultString = characterNames.join(' and ');
  return resultString;
};
...
let findShortest = (data) => {
  const shortestCharacter = data.reduce((shortest, current) => {
    const shortestHeight = parseFloat(shortest.height) || Infinity;
    const currentHeight = parseFloat(current.height) || Infinity;
    return shortestHeight < currentHeight ? shortest : current;
  });
