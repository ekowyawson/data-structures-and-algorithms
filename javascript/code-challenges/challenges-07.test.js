...
const sortNames = (arr) => {
  const compareNames = (a, b) => {
    // If the uppercase names are equal, compare the original case
    if (a < b) {
      return -1;
    }
    if (a > b) {
...
const sortByPrice = (arr) => {
  const comparePrices = (a, b) => a.price - b.price;
  return arr.sort(comparePrices);
};
...
const sortNumbersByLength = (arr) => {
  const compareLengths = (a, b) => a.toString().length - b.toString().length;
  return arr.sort(compareLengths);
};
...
const sortPeople = (arr) => {
  const compareLastNames = (a, b) => a.lastName.localeCompare(b.lastName);
  return arr.sort(compareLastNames);
};
